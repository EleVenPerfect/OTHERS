...
static void NVIC_Configuration(void)
{
 ...
 /**
  * @brief  配置 PA0 PF11为线中断口，并设置中断优先级
  * @param  无
  * @retval 无
  */
void EXTI_Pxy_Config(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin = EXTI_GPIO_PIN1;         //选择按键的引脚 
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;	          //设置引脚为输入模式 
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_DOWN;          //设置引脚为上拉
  GPIO_Init(EXTI_GPIO_PORT1, &GPIO_InitStructure);      //使用上面的结构体初始化按键
	
  
	RCC_AHB1PeriphClockCmd( EXTI_GPIO_CLK2, ENABLE );
	GPIO_InitStructure.GPIO_Pin = EXTI_GPIO_PIN2;         //选择按键的引脚
	GPIO_Init(EXTI_GPIO_PORT2, &GPIO_InitStructure);      //使用上面的结构体初始化按键
