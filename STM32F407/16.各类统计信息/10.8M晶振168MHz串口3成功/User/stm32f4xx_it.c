...
extern OS_TCB	 AppTaskUsart2TCB;
extern OS_TCB	 AppTaskUsart3TCB;
extern OS_TCB	 AppTaskKey1TCB;
extern OS_TCB	 AppTaskKey2TCB;
...
void USART_INT_FUN(void)
{
	...
	{
		* p_mem_blk = USART_ReceiveData ( USARTx );     //获取接收到的数据
		//printf("%c",p_mem_blk[0]);
    /* 发布任务消息到任务 AppTaskUsart */
		OSTaskQPost ((OS_TCB      *)&AppTaskUsart1TCB,      //目标任务的控制块
...
void USART2_INT_FUN(void)
{
	...
	if ( USART_GetITStatus ( USART2x, USART_IT_RXNE ) != RESET )
	{
		* p_mem_blk = USART_ReceiveData ( USART2x );     //获取接收到的数据
		//printf("%c",p_mem_blk[0]);
    /* 发布任务消息到任务 AppTaskUsart */
		OSTaskQPost ((OS_TCB      *)&AppTaskUsart2TCB,      //目标任务的控制块
								 (void        *)p_mem_blk,             //消息内容的首地址
								 (OS_MSG_SIZE  )1,                     //消息长度
								 (OS_OPT       )OS_OPT_POST_FIFO,      //发布到任务消息队列的入口端
								 (OS_ERR      *)&err);                 //返回错误类型
...
void EXTI_INT_FUNCTION2 (void)
{
	...
	{
		/* 发送任务信号量到任务 AppTaskKey2 */
		OSTaskSemPost((OS_TCB  *)&AppTaskKey2TCB,   //目标任务
									(OS_OPT   )OS_OPT_POST_NONE, //没选项要求
									(OS_ERR  *)&err);            //返回错误类型		
