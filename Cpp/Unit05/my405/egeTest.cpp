...
//			std::cout<<x1_<<" "<<y1_<<" "<<x2_-x1_<<" "<<y2_-y1_<<std::endl;
		...
		xyprintf(10,this->screen_->getWidth()-160,"screen init ok!");

		xyprintf(10,this->screen_->getWidth()-140,"build rectangle ok!");
		
		char s[5];		//数字转换缓存 
		sprintf(s, "%d", this->x2_ - this->x1_);
		xyprintf(x2_,y2_,"Width:");
		xyprintf(x2_+60,y2_,s);
		sprintf(s, "%d", this->y2_ - this->y1_);
		xyprintf(x2_,y2_+20,"Height:");
		xyprintf(x2_+60,y2_+20,s);
	}
